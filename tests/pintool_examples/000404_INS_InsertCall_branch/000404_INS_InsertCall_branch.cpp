...
VOID instr_branch(ADDRINT ins_addr, THREADID tid,  BOOL branch_taken, ADDRINT branch_target, ADDRINT branch_fallthrough ) { 
  icount++; 
  std::cout << "===================================================="<< std::endl << std::flush;
  std::cout << ins_addr << " : " << addr2disassemble[ins_addr] << std::endl << std::flush;
  std::cout << "Thread ID: " << tid << std::endl << std::flush;
  std::cout << "BRANCH TAKEN: "  << branch_taken << std::endl << std::flush;
  std::cout << "BRANCH TARGET: "  << branch_target << std::endl << std::flush;
  std::cout << "BRANCH FALLTHROUGH: "  << branch_fallthrough << std::endl << std::flush;
  std::cout << "===================================================="<< std::endl << std::flush;
}
...
VOID Instruction(INS ins, VOID *v)
{
  ...
  /* } */
   /* if ( INS_IsBranch(ins) ) { */
    ...
    INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)instr_branch, IARG_ADDRINT, ins_addresss, IARG_THREAD_ID, IARG_BRANCH_TAKEN, IARG_BRANCH_TARGET_ADDR, IARG_FALLTHROUGH_ADDR,   IARG_END);
  /* } */
